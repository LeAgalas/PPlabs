...
ts = []
with open("../scripts/parallel_results.txt", "r") as f:
    lines = f.readlines()
    for line in lines:
...
def get_plot(ylabel, unit, unitbound, threadbound, filename, all_ts, ymax, cores=8):
    ...
    step = 2
    p.set_xticks(range(0, datalen + 1, step))
    p.set_xticklabels([str(x) for x in range(0, datalen + 1, step)])
    ...
    p.axvline(x=cores, ymin=0, ymax=m2 / m1,
...
get_plot("AvgTime", "ms", unitbound, threadbound,
         "AvgTime.png", all_ts, max(ts_ds[0][1], exp_ts[0][1]))
...
get_plot("Acceleration", "times", unitbound, threadbound,
         "Acceleration.png", all_ts, max(acc_ds[cores-1][1], exp_acc[cores-1][1]))
...
get_plot("Efficiency", "thread^-1", unitbound, threadbound,
         "Efficiency.png", all_ts, max(eff_ds[0][1], exp_eff[0][1]))
